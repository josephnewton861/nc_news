...
describe("app", () => {
  ...
  describe("/api", () => {
    describe("GET", () => {
      it("status: 200 Responds with JSON describing all the available endpoints on the API", () => {
        return request(app)
          .get("/api")
          .expect(200)
          .then((response) => {
      ...
      it("status: 404 invalid input path for /api", () => {
        return request(app)
          .get("/invalid")
          .expect(404)
          .then(({ body: { msg } }) => {
    ...
    describe("INVALID PATHS", () => {
        ...
        describe("/:username", () => {
          ...
          describe("GET", () => {
            ...
            it("status: 404 valid input but username searched for does not exist in db", () => {
              return request(app)
                .get("/api/users/coder1000")
                .expect(404)
                .then(({ body: { msg } }) => {
                  expect(msg).to.equal(
                    "username coder1000 does not exist in database"
                  );
                });
            });
          });
      ...
      describe("/articles", () => {
        describe("GET", () => {
          ...
          it("status: 200 Able to accept a topic query", () => {
            return request(app)
          ...
          describe("PATCH", () => {
            ...
            it("status 404: valid input however article_id inputted does not exist in db", () => {
              return request(app)
                ...
                .then(({ body: { msg } }) => {
                  expect(msg).to.equal(
                    "article_id 1000 does not exist in database"
                  );
                });
            });
            ...
            describe("POST", () => {
              it("status: 201 responds with an inserted comment object", () => {
                return request(app)
                  ...
                  .expect(201)
                  .then(({ body: { comment } }) => {
                    expect(comment).to.contain.keys("author", "body");
                  });
              });
      ...
      describe("/comments", () => {
        describe("/:comment_id", () => {
          ...
          describe("PATCH", () => {
            ...
            it("status: 200 Responds with no incrementation if comment body is empty", () => {
              return request(app)
                .patch("/api/comments/1")
                .send({})
                .expect(200)
            ...
            it("status: 404 Valid input however comment id inputted does not exist in db for a comment with a body", () => {
              return request(app)
                ...
                .then(({ body: { msg } }) => {
                  expect(msg).to.equal(
                    "comment_id 1000 does not exist in database"
                  );
                });
            });
            it("status: 404 Valid input however comment id inputted does not exist in db for a comment without a body", () => {
              return request(app)
                .patch("/api/comments/1000")
                .send({})
                .expect(404)
